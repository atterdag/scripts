...
# the dict function is missing from the wsadmin jython, so we have to make our own
def dict(sequence):
  resultDict = {}
...
def printUsage():
    ...
    print '--serverId <WAS server ID>'
    print '--serverIdPassword <WAS server password>'
...
serverId                = optdict.get('--serverId', '')
serverIdPassword        = optdict.get('--serverIdPassword', '')
primaryAdminId          = optdict.get('--primaryAdminId', serverId)
...
print 'setting realm primary ID to ' + primaryAdminId
result = AdminTask.configureAdminWIMUserRegistry('[-autoGenerateServerId true -primaryAdminId ' + primaryAdminId + ' -serverId ' + serverId + ' -serverIdPassword ' + serverIdPassword + ' -ignoreCase true -customProperties  -verifyRegistry false ]')
...
#print 'set realm server primary ID'
