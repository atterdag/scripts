...
pki_ca_signing_csr_path = /root/.dogtag/pki-tomcat/ca_signing.csr
...
EOF
sudo pkispawn -s CA -f /var/lib/openstack/dogtag-step1.cfg

sudo cp -f /root/.dogtag/pki-tomcat/ca_signing.csr \
...
sudo openssl x509 \
  -in ${SSL_BASE_DIR}/${SSL_ROOT_CA_STRICT_NAME}/certs/${SSL_ROOT_CA_STRICT_NAME}.crt \
  -out /root/.dogtag/pki-tomcat/${SSL_ROOT_CA_STRICT_NAME}.crt
sudo openssl x509 \
  -in ${SSL_BASE_DIR}/${SSL_ROOT_CA_STRICT_NAME}/certs/${SSL_INTERMEDIATE_CA_TWO_STRICT_NAME}.crt \
  -out /root/.dogtag/pki-tomcat/ca_signing.crt
...
pki_admin_uid = caadmin
pki_audit_signing_nickname=${SSL_INTERMEDIATE_AUDIT_TWO_STRICT_NAME}
pki_ca_signing_cert_path = /root/.dogtag/pki-tomcat/ca_signing.crt
pki_ca_signing_csr_path = /root/.dogtag/pki-tomcat/ca_signing.csr
pki_ca_signing_key_algorithm = SHA256withRSA
pki_ca_signing_key_size = 4096
pki_ca_signing_key_type = rsa
...
pki_cert_chain_path = /root/.dogtag/pki-tomcat/${SSL_ROOT_CA_STRICT_NAME}.crt
pki_client_database_purge = False
pki_ds_base_dn = dc=ca,dc=pki,${DS_SUFFIX}
pki_ds_database = ca
pki_external = True
...
pki_security_domain_user = caadmin
EOF
sudo pkispawn -s CA -f /var/lib/openstack/dogtag-step2.cfg
