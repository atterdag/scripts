...
if [[ $CONTROLLER_FQDN != $COMPUTE_FQDN ]]; then
  ...
  sudo crudini --set /etc/neutron/plugins/ml2/linuxbridge_agent.ini linux_bridge physical_interface_mappings "provider:${NETWORK_INTERFACE}"
  ...
  sudo crudini --set /etc/neutron/plugins/ml2/linuxbridge_agent.ini linux_bridge bridge_mappings "provider:${NETWORK_INTERFACE}"
