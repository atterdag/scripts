...
CONTEXT=QUANTUM
etcdctl --username admin:"$ETCD_ADMIN_PASS" set /variables/${CONTEXT}_HOST_NAME 'quantum'
etcdctl --username admin:"$ETCD_ADMIN_PASS" set /variables/${CONTEXT}_IP_ADDRESS '192.168.1.128'
etcdctl --username admin:"$ETCD_ADMIN_PASS" set /variables/${CONTEXT}_FQDN "$(etcdctl get variables/${CONTEXT}_HOST_NAME).$(etcdctl get variables/PC_DNS_DOMAIN)"
