...
CONTEXT=MASTER01
etcdctl --username admin:"$ETCD_ADMIN_PASS" set /variables/${CONTEXT}_HOST_NAME 'master01'
etcdctl --username admin:"$ETCD_ADMIN_PASS" set /variables/${CONTEXT}_IP_ADDRESS '192.168.100.11'
etcdctl --username admin:"$ETCD_ADMIN_PASS" set /variables/${CONTEXT}_FQDN "$(etcdctl get variables/${CONTEXT}_HOST_NAME).$(etcdctl get variables/MAAS_DNS_DOMAIN)"
