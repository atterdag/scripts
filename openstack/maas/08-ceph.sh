...
echo '***'
if [ ! -d /mnt/${K8S_CEPH_FS_NAME} ]; then sudo mkdir -p /mnt/${K8S_CEPH_FS_NAME}; fi

if [[ -f /usr/bin/wslsys ]]; then
  # Using FUSE - WSL
  sudo ceph-fuse -m ${CEPH_MON_ONE_IP_ADDRESS}:${CEPH_MONITOR_SERVER_PORT},${CEPH_MON_TWO_IP_ADDRESS}:${CEPH_MONITOR_SERVER_PORT} /mnt/${K8S_CEPH_FS_NAME}
else
  # Using the kernel module
  sudo mount -t ceph ${CEPH_MON_ONE_IP_ADDRESS}:${CEPH_MONITOR_SERVER_PORT},${CEPH_MON_TWO_IP_ADDRESS}:${CEPH_MONITOR_SERVER_PORT}:/ /mnt/${K8S_CEPH_FS_NAME} -o name=admin,secret=${CEPH_ADMIN_CLIENT_KEY}
fi
