...
sudo wget \
  --continue \
  --output-document=${OPENSTACK_IMAGES_DIRECTORY}/ubuntu-18.04-server-cloudimg-amd64.img \
  https://cloud-images.ubuntu.com/bionic/current/bionic-server-cloudimg-amd64.img

openstack image create \
  --container-format bare \
  --disk-format qcow2 \
  --file ${OPENSTACK_IMAGES_DIRECTORY}/ubuntu-18.04-server-cloudimg-amd64.img \
  --public \
