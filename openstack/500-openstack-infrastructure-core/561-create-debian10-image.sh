...
sudo wget \
  --continue \
  --output-document=${OPENSTACK_CONFIGURATION_DIRECTORY}/debian-10-openstack-amd64.qcow2 \
...
sudo --preserve-env openstack image create \
  ...
  --disk-format qcow2 \
  --file ${OPENSTACK_CONFIGURATION_DIRECTORY}/debian-10-openstack-amd64.qcow2 \
  --public \
