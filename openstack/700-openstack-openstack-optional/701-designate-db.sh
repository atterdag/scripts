...
##############################################################################
cat << EOF | sudo tee ${OPENSTACK_CONFIGURATION_DIRECTORY}/designate.sql
...
EOF
sudo chmod 0600 ${OPENSTACK_CONFIGURATION_DIRECTORY}/designate.sql
sudo cat ${OPENSTACK_CONFIGURATION_DIRECTORY}/designate.sql | sudo mysql --host=localhost --user=root --password="${ROOT_DBPASS}"
