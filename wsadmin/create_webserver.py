...
optlist, args = getopt.getopt(sys.argv, 'x', [
    'fqdn=', 'ihsInstallRoot=', 'plgInstallRoot=', 'adminPort=',
    'adminUserID=', 'adminPasswd=', 'adminProtocol=', 'operatingSystem='
])
...
for unmanagedNode in unmanagedNodes:
    if (unmanagedNode == nodeName):
        ...
        for webserver in webservers:
            ...
            AdminTask.deleteKeyStore(
                '[-keyStoreName CMSKeyStore -scopeName (cell):' + cell +
                ':(node):' + nodeName + ':(server):' + webserverName + ' ]')
            print 'deleting previous server: ' + nodeName + '/' + webserverName
            AdminTask.deleteWebServer('[-serverName ' + webserverName +
                                      ' -nodeName ' + nodeName + ']')
...
AdminTask.createUnmanagedNode('[-nodeName ' + nodeName + ' -hostName ' + fqdn +
                              ' -nodeOperatingSystem ' + operatingSystem + ']')
print 'creating new web server:     ' + nodeName + '/webserver1'
AdminTask.createWebServer(
    hostname + 'UnmanagedNode',
    '[-name webserver1 -templateName IHS -serverConfig [-webPort 80 -serviceName -webInstallRoot '
    + ihsInstallRoot +
    ' -webProtocol HTTPS -configurationFile  -errorLogfile  -accessLogfile  -pluginInstallRoot '
    + plgInstallRoot + ' -webAppMapping ALL] -remoteServerConfig [-adminPort '
    + adminPort + ' -adminUserID ' + adminUserID + ' -adminPasswd ' +
    adminPasswd + ' -adminProtocol ' + adminProtocol + ']]')
...
default_hostId = AdminConfig.getid(
    '/Cell:' + cell + '/VirtualHost:default_host/')
...
for default_hostIdAlias in default_hostIdAliases.split():
    default_hostIdAliasHostname = AdminConfig.showAttribute(
        default_hostIdAlias, 'hostname')
    default_hostIdAliasPort = AdminConfig.showAttribute(
        default_hostIdAlias, 'port')
    if (default_hostIdAliasHostname == '*' and
        (default_hostIdAliasPort == '80' or default_hostIdAliasPort == '443')):
...
result = AdminConfig.create(
    'HostAlias',
    AdminConfig.getid('/Cell:' + cell + '/VirtualHost:default_host/'),
    '[[port "80"] [hostname "*"]]')
...
print '##############################################################################'
print
webserverPluginSettingsIds = AdminConfig.list(
    'WebserverPluginSettings').splitlines()
for webserverPluginSettingsId in webserverPluginSettingsIds:
    serverIOTimeout = AdminConfig.showAttribute(webserverPluginSettingsId,
                                                'ServerIOTimeout')
    print "modifying ServerOITimeout from " + serverIOTimeout + " to 0"
    result = AdminConfig.modify(webserverPluginSettingsId,
                                '[[ServerIOTimeout "0"]]')
