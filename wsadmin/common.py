...
def configureDataSourceConnectionPool(dataSourceName, minConnections, maxConnections, agedTimeout):
  ...
  attributes = [['minConnections', minConnections], ['maxConnections', maxConnections], ['agedTimeout', agedTimeout]]
...
def configureHpel(server,nodeName):
  ...
  result = AdminConfig.modify(highPerformanceExtensibleLogging,'[[enable true]]')
  hpelLog = AdminConfig.showAttribute(highPerformanceExtensibleLogging,'hpelLog')
  hpelTextLog = AdminConfig.showAttribute(highPerformanceExtensibleLogging,'hpelTextLog')
  hpelTrace = AdminConfig.showAttribute(highPerformanceExtensibleLogging,'hpelTrace')
...
def configureJvmLogRotation(server):
  ...
  for logtype in logtypes:
    ...
    result = AdminConfig.modify(log, [['baseHour', 24],['maxNumberOfBackupFiles', 30],['rolloverPeriod', 24],['rolloverType', 'TIME']])
...
def configureJvmRuntimeTrace(server,level):
  ...
  reult = AdminControl.setAttribute(traceServiceCompleteObjectName, 'traceSpecification', level)
def configureTraceLog(server,number,size):
  ...
  result = AdminConfig.modify(traceLog, [['maxNumberOfBackupFiles',number],['rolloverSize',size]])
...
def setMimeEntry(type, extensions):
  ...
  for mimeEntry in mimeEntries.splitlines():
    ...
    if mimeEntryType == type:
      result = AdminConfig.modify(mimeEntry, [['extensions', extensions]])
      return
  result = AdminConfig.create('MimeEntry', virtualHostId, [['type', type], ['extensions', extensions]])
def setPluginProperty(pluginProperty, name, value, description='""'):
  ...
  if create == 'true':
    ...
    result = AdminConfig.create('Property', pluginProperty, [['name', name],['value', value],['description', description],['required', 'false']])
  else:
    print 'modifying existing plugin custom property ' + name + ' with value ' + value
    result = AdminConfig.modify(matchingProperty, [['name', name],['value', value],['description', description ],['required', 'false']])
    ...
    if pmiRmFilterType == type:
      ...
      for filterValue in filterValues[0]:
        ...
        if filterValueValue == value:
          ...
          result = AdminConfig.modify(filterValue, '[[enable "' + enable + '"] [value "' + value + '"]]')
          filterExist = 'true'
          return
      print 'creating new ' + type + ' filter value, ' + value + ', to ' + enable
      result = AdminConfig.create('PMIRMFilterValue', pmiRmFilter, '[[enable "' + enable + '"] [value "' + value + '"]]')
...
def setSessionTuning(server, timeout, inMemorySessions):
  ...
  AdminConfig.modify(tuningParams, [['invalidationTimeout', timeout], ['maxInMemorySessionCount', inMemorySessions]])
