...
for node in nodes:
    ...
    variableSubstitutionEntries = AdminConfig.list('VariableSubstitutionEntry',
                                                   nodeId).splitlines()
    variableSubstitutionEntryNameFound = 'false'
    for variableSubstitutionEntry in variableSubstitutionEntries:
        variableSubstitutionEntryName = AdminConfig.showAttribute(
    ...
    if variableSubstitutionEntryNameFound == 'false':
        ...
        result = AdminConfig.create(
            'VariableSubstitutionEntry', '(cells/' + cell + '/node/' + node +
            '|variables.xml#VariableMap_1)',
            '[[symbolicName "WAS_NODE_NAME"] [value "' + node + '"]]')
