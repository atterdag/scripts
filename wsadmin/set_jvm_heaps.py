...
optlist, args = getopt.getopt(sys.argv, 'x', [
    'dmgrHeapSizeMax=', 'dmgrHeapSizeMin=', 'naHeapSizeMax=', 'naHeapSizeMin=',
    'asHeapSizeMax=', 'asHeapSizeMin=', 'asJvmNurserySize='
])
...
for node in nodes:
    ...
    dmgrs = AdminTask.listServers('[-serverType DEPLOYMENT_MANAGER -nodeName '
                                  + nodeName + ']').splitlines()
    for dmgr in dmgrs:
        setJavaVirtualMachineProperty(
            dmgr, [['initialHeapSize', dmgrHeapSizeMin], [
                'maximumHeapSize', dmgrHeapSizeMax
    ...
    for nodeagent in nodeagents:
        setJavaVirtualMachineProperty(
            nodeagent, [['initialHeapSize', naHeapSizeMin], [
                'maximumHeapSize', naHeapSizeMax
            ], ['verboseModeGarbageCollection', 'false']])
    ...
    for server in servers:
        ...
        setJavaVirtualMachineProperty(server,
                                      [['initialHeapSize', asHeapSizeMin],
                                       ['maximumHeapSize', asHeapSizeMax]])
        setJvmGenericArguements(nodeName, serverName,
                                '-Xmn' + asJvmNurserySize + 'M')
