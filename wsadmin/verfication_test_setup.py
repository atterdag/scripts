...
result = AdminTask.createCluster('[-clusterConfig [-clusterName TestCluster -preferLocal true]]')
...
for managedNodeName in managedNodeNames:
  if ( isFirstMemberMade != 'true' ):
    ...
    result = AdminTask.createClusterMember('[-clusterName TestCluster -memberConfig [-memberNode ' + managedNodeName + ' -memberName testserver_' + managedNodeName + ' -memberWeight 2 -genUniquePorts true -replicatorEntry false] -firstMember [-templateName default -nodeGroup DefaultNodeGroup -coreGroup DefaultCoreGroup -resourcesScope cluster]]')
    isFirstMemberMade = 'true'
  else:
    print 'create a cluster member testserver_' + managedNodeName + ' on each node ' + managedNodeName
    result = AdminTask.createClusterMember('[-clusterName TestCluster -memberConfig [-memberNode ' + managedNodeName + ' -memberName testserver_' + managedNodeName + ' -memberWeight 2 -genUniquePorts true -replicatorEntry false]]')
...
result = AdminApp.install(os.environ['WAS_HOME'] + '/installableApps/DefaultApplication.ear')
...
for managedNodeName in managedNodeNames:
  ...
  for applicationServer in applicationServers:
    ...
    if applicationServerName == 'testserver_' + managedNodeName:
      ...
      appManager = AdminControl.queryNames('type=ApplicationManager,process=testserver_' + managedNodeName + ',*')
