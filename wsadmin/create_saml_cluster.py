...
result = AdminTask.createCluster(
    '[-clusterConfig [-clusterName SamlCluster -preferLocal true]]')
...
for managedNodeName in managedNodeNames:
    if (isFirstMemberMade != 'true'):
        ...
        result = AdminTask.createClusterMember(
            '[-clusterName SamlCluster -memberConfig [-memberNode ' +
            managedNodeName + ' -memberName SamlServer_' + managedNodeName +
    ...
    for applicationServer in applicationServers:
        ...
        if applicationServerName == 'SamlServer_' + managedNodeName:
            ...
            appManager = AdminControl.queryNames(
                'type=ApplicationManager,process=SamlServer_' +
                managedNodeName + ',*')
