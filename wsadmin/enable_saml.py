...
optlist, args = getopt.getopt(sys.argv, 'x', [
    'idpFqdn=', 'appFqdn=', 'appContextRoot=', 'principalName=', 'acsUrl=',
    'errorPage=', 'targetUrl=', 'filter=', 'databaseHostTwo=',
    'databasePortTwo='
])
...
errorPage = optdict.get('--errorPage', 'https://' + idpFqdn +
                        '/adfs/ls/IdpInitiatedSignOn.aspx?loginToRp=' + acsUrl)
targetUrl = optdict.get('--targetUrl', 'https://' + lbFqdn + appContextRoot)
filter = optdict.get('--filter', 'request-url%=' + appContextRoot)

# check for required values
if (idpFqdn == '' or appFqdn == '' or appContextRoot == ''):
    printUsage()
...
result = AdminTask.addSAMLTAISSO(
    '-enable true -acsUrl ' + acsUrl +
    ' -trustStoreName SAMLKeyStore -keyStoreName SAMLKeyStore -keyAlias samlSP-certificate -keyName samlSP-certificate -keyPassword WebAS -errorPage '
    + errorPage + ' -idMap localRealm')

saveConfiguration()

print 'setting additional SAML TAI properties'
result = AdminTask.configureInterceptor(
    '[-interceptor com.ibm.ws.security.web.saml.ACSTrustAssociationInterceptor -customProperties ["sso_1.sp.targetUrl='
    + targetUrl + '","sso_1.sp.principalName=' + principalName +
    '","sso_1.sp.acsErrorPage=' + errorPage + '","sso_1.sp.useRealm=' +
