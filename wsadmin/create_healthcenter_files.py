...
authenticationFile.write(
    healthcenterUsername + ' ' + healthcenterPassword + '\n')
...
authorizationFile.write(
    healthcenterUsername + ' ' + healthcenterPrivilege + '\n')
authorizationFile.flush()
authorizationFile.close()

if os.path.exists(
        os.environ['CONFIG_ROOT'] + '/cells/' + cell + '/healthcenter.jks'):
    print 'deleting existing ' + os.environ['CONFIG_ROOT'] + '/cells/' + cell + '/healthcenter.jks'
    result = AdminTask.deleteKeyStore(
...
result = AdminTask.createKeyStore(
    ...
    ' -keyStoreType JKS    -keyStoreInitAtStartup false -keyStoreReadOnly false -keyStoreStashFile false -keyStoreUsage SSLKeys ]'
)
...
result = AdminTask.createSelfSignedCertificate(
    '[-signatureAlgorithm SHA256withRSA -keyStoreName HealthCenterKeystore -keyStoreScope (cell):'
    + cell +
    ' -certificateAlias HealthCenterCertificate -certificateVersion -certificateSize 2048 -certificateCommonName HealthCenterCertificate -certificateValidDays 365 ]'
)
...
result = AdminTask.extractSignerCertificate(
    '[-keyStoreName HealthCenterKeystore -keyStoreScope (cell):' + cell +
    ' -certificateFilePath /tmp/root.cer -base64Encoded true -certificateAlias root ]'
)
...
result = AdminTask.extractCertificate(
    '[-certificateFilePath /tmp/healthcenter.cer -base64Encoded true -certificateAlias HealthCenterCertificate -keyStoreName HealthCenterKeystore -keyStoreScope (cell):'
    + cell + ' ]')
