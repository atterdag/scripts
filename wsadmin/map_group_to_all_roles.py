...
def printUsage():
    ...
    print '-f /tmp/map_group_to_all_roles.py'
...
print '##############################################################################'
result = AdminTask.mapGroupsToAdminRole(
    '[-roleName adminsecuritymanager -accessids [group:' + realmName + '/' +
    groupDN + '] -groupids [' + groupID + '@' + realmName + ']]')
result = AdminTask.mapGroupsToAdminRole(
    '[-roleName administrator        -accessids [group:' + realmName + '/' +
    groupDN + '] -groupids [' + groupID + '@' + realmName + ']]')
result = AdminTask.mapGroupsToAuditRole(
    '[-roleName auditor              -accessids [group:' + realmName + '/' +
    groupDN + '] -groupids [' + groupID + '@' + realmName + ']]')
result = AdminTask.mapGroupsToAdminRole(
    '[-roleName iscadmins            -accessids [group:' + realmName + '/' +
    groupDN + '] -groupids [' + groupID + '@' + realmName + ']]')
result = AdminTask.mapGroupsToAdminRole(
    '[-roleName configurator         -accessids [group:' + realmName + '/' +
    groupDN + '] -groupids [' + groupID + '@' + realmName + ']]')
result = AdminTask.mapGroupsToAdminRole(
    '[-roleName deployer             -accessids [group:' + realmName + '/' +
    groupDN + '] -groupids [' + groupID + '@' + realmName + ']]')
result = AdminTask.mapGroupsToAdminRole(
    '[-roleName monitor              -accessids [group:' + realmName + '/' +
    groupDN + '] -groupids [' + groupID + '@' + realmName + ']]')
result = AdminTask.mapGroupsToAdminRole(
    '[-roleName operator             -accessids [group:' + realmName + '/' +
    groupDN + '] -groupids [' + groupID + '@' + realmName + ']]')

synchronizeActiveNodes()

print 'refreshing roles'
authGrpMgrDmgr = AdminControl.completeObjectName(
    'WebSphere:type=AuthorizationGroupManager,process=dmgr,*')
