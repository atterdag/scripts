...
for managedNodeName in managedNodeNames:
  ...
  for applicationServer in applicationServers:
    ...
    if applicationServerName == 'testserver_' + managedNodeName:
      ...
      result = AdminControl.invoke(appManager, 'stopApplication','DefaultApplication.ear')
...
for managedNodeName in managedNodeNames:
  ...
  for applicationServer in applicationServers:
    ...
    if applicationServerName == 'testserver_' + managedNodeName:
      ...
      result = AdminControl.stopServer(applicationServerName,managedNodeName)
      print 'deleting ' + applicationServerName + ' on ' + managedNodeName
      result = AdminTask.deleteServer('[-serverName ' + applicationServerName + ' -nodeName ' + managedNodeName + ' ]')
