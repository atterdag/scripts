...
sslConfigs = AdminTask.listSSLConfigs(
    '[-all true -displayObjectName true ]').splitlines()
for sslConfig in sslConfigs:
    alias = AdminConfig.showAttribute(sslConfig, 'alias')
    if (alias == 'CellDefaultSSLSettings'
            or alias == 'NodeDefaultSSLSettings'):
        ...
        result = AdminTask.modifySSLConfig(
            '[-alias ' + alias + ' -scopeName ' + scopeName +
